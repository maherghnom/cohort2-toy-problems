...
function Jar() {
  ...
  var total=0;
  return {
    add:function(quantity,fruit){
    total+=quantity;
    if ( Array.isArray(jar[fruit])){
      jar[fruit].push(quantity)
    } else {
        jar[fruit] = [quantity]
      }
    ...
    getConcentration:function(fruit){
      var fruittotal=0;
      for (var i = 0 ; i < jar[fruit].length ; i++){
        fruittotal+=jar[fruit][i]
      }
      var conc=fruittotal/total;
      return conc;
    },
    pour:function(quantity){
      total=total-quantity;
      var key=Object.keys(jar);

      var fruittotal=0;

      for (var i = 0 ; i < key.length ; i++){
                jar[i][0]=100/key.length
      }
